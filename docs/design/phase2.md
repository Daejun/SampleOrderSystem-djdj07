...
### 4. 설계 대비 구조 변경: `JsonStore`의 `cache_` 분리 (코드 리뷰에서 발견)

설계 시점에는 `samples()`/`orders()`가 `doc_.root()`를 직접 참조하는 구조였으나, 실제 구현은 별도의
`cache_`(`ordered_json`)를 두고 `save()` 호출 시점에 `cache_`를 `doc_`에 복사해 넣은 뒤 파일에 쓰는
방식으로 변경되었다. 동작·테스트 결과에는 영향이 없으나(14/14 통과), 설계 문서와 코드가 어긋난 상태로
남아있던 것을 Phase 2 완료 후 코드 리뷰에서 발견했다. 앞으로 `JsonStore`를 다루는 Phase(3 이상)는
`doc_.root()`가 아니라 `cache_`가 실제 데이터를 들고 있다는 점을 전제로 설계한다.

### 5. 콘솔 입력 파싱과 I/O 결합 (코드 리뷰에서 발견, 테스트 공백)

`MainController::runSampleMenu()`는 `std::cin` 읽기와 `std::stod` 파싱/검증을 한 함수 안에서 처리한다.
`log/phase2.md`의 "검증하지 못한 내용"에도 명시되어 있듯, 이 때문에 표준입력을 모킹하지 않는 한
자동화된 단위 테스트를 작성할 수 없다. Phase 3(주문 접수)부터는 입력 항목이 늘어나므로, 파싱/검증
로직을 `std::cin`과 분리된 순수 함수로 추출해 테스트 가능하게 만드는 것을 권장한다 (강제 아님 — Phase 3
설계 시 채택 여부 결정).
