...
- **구현 완료** (`log/phase12.md` 참고): `SampleSearchFieldTest`/`SampleRegisterBoundaryTest`(`TEST_P`, 총 12케이스), `ProductionQueueTest`/`JsonStoreTest`에 방어적 분기 테스트 3개 추가(총 102개). `scripts/coverage.sh -b` 실행 결과, 우선순위 1~3 대상 분기 전부 양쪽 경로(참/거짓) 최소 1회 실행 확인. `tc.md` 3부 신설.
