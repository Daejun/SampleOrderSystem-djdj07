...
```
res/                    # 원본 과제 PDF (요구사항의 최초 출처, 변경하지 않음)
prd.md                  # PDF를 바탕으로 한 전체 기능 명세 및 도메인 규칙 ("무엇을 만드는가")
PLAN.md                 # prd.md를 Phase로 분해한 개발 계획 ("어떤 순서로 만드는가")
docs/design/phaseN.md   # 각 Phase 착수 전 작성하는 상세 설계 ("어떻게 만드는가")
log/phaseN.md           # 각 Phase 완료 후 기록하는 4대 Verify 결과 ("실제로 무엇을 만들었고 어떻게 검증했는가")
tc.md                   # Phase를 가로지르는 전체 테스트 케이스 인벤토리 + 특정 로직(재고/타이밍 등)의 심화 시나리오 카탈로그
CLAUDE.md               # 이 파일 - 위 문서들을 어떻게 만들고 참조할지에 대한 Agent 행동 규칙
```
...
res/ (요구사항 원본)
  -> prd.md (기능 명세)
       -> PLAN.md (Phase 분해 + Phase별 목표/범위/완료기준)
            -> docs/design/phaseN.md (Phase 착수 전 상세 설계, 사람 리뷰)
                 -> 구현 (Verify 통과)
                      -> log/phaseN.md (구현 내용 + 4대 Verify 결과 기록)
                      -> tests/*.cpp (실제 gtest 케이스)
                           -> tc.md (여러 Phase/테스트 파일을 가로지르는 테스트 케이스 카탈로그, tests/*.cpp와 1:1로 매핑)
```

- 이전 Phase의 **구현(코드 리뷰) 중 발견된 사항**은 `PLAN.md`의 다음 Phase 항목에 "설계 시 반드시 확정할 사항"으로 먼저 반영한 뒤, 해당 Phase의 `docs/design/phaseN.md`에서 구체적으로 해소한다 (예: Phase 1→2의 공유 저장소 이슈, Phase 3→4의 Controller 재사용 방침, Phase 4→5의 `store_.save()` 중복 호출).
- `PLAN.md`의 "미결 사항"은 근거 문서가 생기면(보통 다음 Phase의 `docs/design/phaseN.md`) 취소선 처리하고 근거를 링크한다.
- `tc.md`는 특정 Phase에 종속되지 않는다 — 여러 테스트 파일에 흩어진 케이스를 한 곳에서 조망하거나(1부: 전체 인벤토리), 특정 도메인 로직 하나를 재고/타이밍 등 여러 축으로 깊게 파고들 때(2부: 시나리오 매트릭스) 사용한다. 새 테스트를 추가하면 `tc.md`도 함께 갱신한다.
- 각 문서는 **자신보다 상위 문서와 충돌해서는 안 된다** (예: `docs/design/phaseN.md`는 `PLAN.md`/`prd.md`와, `log/phaseN.md`는 해당 `docs/design/phaseN.md`와 일치해야 함). 충돌이 발견되면 Document Consistency Verify에서 다루고 사람에게 확인한다.
...
- `tc.md` — 전체 테스트 케이스 인벤토리 및 재고/타이밍 등 심화 시나리오 카탈로그 (테스트 추가/변경 시 갱신)
- `res/` — 원본 과제 PDF
